...
/**
 * MethodOptions: Renders method-specific configuration controls.
 */
export default function MethodOptions({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {/* Least-Squares sub-method selection */}
      {method === 'leastSquares' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <label style={labelStyle}>Fit Type</label>
          {[
            { id: 'linear', label: 'Linear (y = a + bx)' },
            { id: 'polynomial', label: 'Polynomial (y = Σcᵢxⁱ)' },
            { id: 'exponential', label: 'Exponential (y = Ceᴬˣ)' },
            { id: 'power', label: 'Power (y = Cxᴬ)' },
      ...
      <div>
        ...
        <input
          ...
          style={inputStyle}
          aria-label="x value for evaluation"
        />
      </div>
    </div>
  );
