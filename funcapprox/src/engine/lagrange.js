 ...
 *
 * Given n+1 data points, construct the unique polynomial of degree <= n
 * that passes through all points using Lagrange basis functions.
 */
import { fmt } from './matrix';

/**
 * Compute Lagrange interpolation with full step-by-step solution.
 *
 * @param {Array<{x: number, y: number}>} points - Data points
 * @param {number|null} xEval - Optional x value to evaluate
 * @returns {object} Result with steps, polynomial, and evaluation
 */
export function lagrangeInterpolation(points, xEval = null) {
  ...
  if (n > 5) {
    steps.push({
      type: 'error',
      latex: `\\textbf{Warning:}\\text{ High-degree polynomial (degree ${n - 1}). Lagrange interpolation may exhibit oscillation (Runge's phenomenon) for } n > 5.`,
    });
  }
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = 0; j < n; j++) {
      if (j === i) continue;
      const diff = points[i].x - points[j].x;
      denom *= diff;
  ...
  for (let i = 0; i < n; i++) {
    const sign = i > 0 ? ' + ' : '';
    terms.push(`${sign}L_{${i}}(x) \\cdot (${fmt(points[i].y)})`);
  }
  polyStep += terms.join('');

  // Compute the actual polynomial coefficients using expanded form
  const coefficients = computePolynomialCoefficients(points, denominators);

  polyStep += `\\\\[8pt]`;
  polyStep += `\\textbf{Simplified form:}\\\\[4pt]`;
  ...
  if (xEval !== null && !isNaN(xEval)) {
    ...
    if (isExtrapolation) {
      evalStep += `\\\\[4pt]\\textbf{Warning:}\\text{ } x = ${fmt(xEval)} \\text{ is outside the data range } [${fmt(xMin)}, ${fmt(xMax)}].`;
      evalStep += `\\text{ Extrapolation may be unreliable.}`;
    }
  ...
  errorStep += `\\text{Note: Exact error requires knowledge of } f^{(${n})}(\\xi) \\text{ which depends on the unknown true function.}`;

  if (xEval !== null && !isNaN(xEval)) {
    let product = 1;
    for (let i = 0; i < n; i++) {
      product *= (xEval - points[i].x);
    }
    errorStep += `\\\\[4pt]\\text{Error bound factor: } \\left|\\prod_{i=0}^{${n - 1}}(${fmt(xEval)} - x_i)\\right| = ${fmt(Math.abs(product), 6)}`;
  }
...
/**
 * Compute polynomial coefficients from Lagrange form.
 * Returns coefficients [c0, c1, c2, ..., cn] where P(x) = c0 + c1*x + c2*x^2 + ...
 */
function computePolynomialCoefficients(points, denominators) {
  ...
  for (let i = 0; i < n; i++) {
    // Build the basis polynomial Li(x) / denominators[i] * yi
    const basisCoeffs = basisPolynomialCoeffs(points, i, denominators[i]);
    const yi = points[i].y;
    for (let k = 0; k < n; k++) {
      coeffs[k] += yi * basisCoeffs[k];
    }
  }
 ...
 * Li(x) = product((x - xj) for j != i) / denominator
 * Returns [c0, c1, ..., cn-1]
 */
function basisPolynomialCoeffs(points, i, denom) {
  ...
  for (let j = 0; j < n; j++) {
    if (j === i) continue;
    // Multiply poly by (x - xj)
    const newPoly = new Array(poly.length + 1).fill(0);
    for (let k = 0; k < poly.length; k++) {
      newPoly[k + 1] += poly[k];           // x * coeff
      newPoly[k] -= points[j].x * poly[k]; // -xj * coeff
    }
    poly = newPoly;
  }
...
/**
 * Evaluate polynomial given coefficients [c0, c1, ..., cn].
 * P(x) = c0 + c1*x + c2*x^2 + ...
 */
...
/** Example datasets for Lagrange interpolation */
